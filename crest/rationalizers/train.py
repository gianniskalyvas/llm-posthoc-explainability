...
def run(args):
    ...
    logger = setup_wandb_logger(args.default_root_dir, project=wandb_project, entity=wandb_entity)
    ...
    if "ckpt" in dict_args.keys() and dict_args["ckpt"] is not None:
        ...
        # Extract Trainer-specific arguments from args
        trainer_kwargs = {
            'logger': logger,
            'callbacks': callbacks,
            'default_root_dir': getattr(args, 'default_root_dir', None),
        ...
        # Add optional trainer args if they exist
        optional_args = ['limit_train_batches', 'limit_val_batches', 'limit_test_batches', 'log_every_n_steps']
        for arg_name in optional_args:
            if hasattr(args, arg_name):
                val = getattr(args, arg_name)
                if val is not None:
                    trainer_kwargs[arg_name] = val
